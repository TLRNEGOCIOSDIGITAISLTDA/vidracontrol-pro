...
const ALL_STATUSES: QuoteStatus[] = [
  "orcado", "enviado", "aguardando", "aprovado", "entregue", "perdido",
];
...
const QuoteList = () => {
  ...
  const [filterPeriod, setFilterPeriod] = useState<FilterPeriod>(
    () => (localStorage.getItem("quotesFilter") as FilterPeriod) || "mes",
  );
  const [filterMonth, setFilterMonth] = useState<string>(() => {
    const now = new Date();
    return (
      localStorage.getItem("quotesFilterMonth") ||
  ...
  const changeFilter = (p: FilterPeriod) => {
    setFilterPeriod(p);
    localStorage.setItem("quotesFilter", p);
  };
  const changeMonth = (key: string) => {
    setFilterMonth(key);
    localStorage.setItem("quotesFilterMonth", key);
  };
  ...
  const chartByStatus = quotesByStatus.filter((d) =>
    CHART_STATUSES.includes(d.status),
  );
  const qtyData = chartByStatus
    .filter((d) => d.count > 0)
    .map((d) => ({ name: d.label, value: d.count, color: d.color }));
  const valueData = chartByStatus
    .filter((d) => d.total > 0)
    .map((d) => ({ name: d.label, value: d.total, color: d.color }));
                ...
                <div key={group.status}>
                  ...
                  <div className="space-y-2">
                    {group.quotes.map((q, i) => {
                      ...
                      const qShowAprovar =
                        qStatus === "enviado" || qStatus === "aguardando";
                      const qShowPerdido =
                        qStatus === "enviado" || qStatus === "aguardando";
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`bg-card rounded-xl shadow-card hover:shadow-elevated transition-shadow ${
                              isNovo
                                ? "ring-2 ring-primary/40 border border-primary/40"
                                : ""
                            }`}
                          >
                            <div className="p-4">
                              <div className="flex items-center justify-between mb-1">
                                ...
                                <span
                                  className={`text-xs px-2 py-0.5 rounded-full font-semibold ${QUOTE_STATUS_BG[group.status]}`}
                                >
                                  {group.label}
                                </span>
                              </div>
